...
superman_decrypt_file() {
  ...
  if [[ "$alias_name" == "help" || "$alias_name" == "-h" || "$alias_name" == "--help" ]]; then
    ...
    echo ""
    echo "Usage: superman_decrypt_file [alias] [encrypted_file] [target_file]"
    echo ""
    echo "Parameters:"
    ...
    echo ""
    echo "Examples:"
    echo "  superman_decrypt_file default /path/to/file.enc /path/to/output"
    echo "  superman_decrypt_file my-alias /path/to/file.enc /path/to/output"
    return 0
  fi
